    ...
    if(this != NULL && pFunc != NULL && order >=0 && order <=1)
    {
    	...
    	do
    	{
    		...
    		for(int i=0; i<newLimite-1; i++)
    		{
    			...
    			if((order && pFunc(voidAux,voidAux2)>0) || (!order && pFunc(voidAux,voidAux2)<0))
    			{
 ...
 * \return int Retorna  (-1) Error: si el puntero a la listas es NULL
                                ( 0) Si ok
 */
LinkedList* ll_filtrer2(LinkedList* this,LinkedList* this2,int (*pFunc)(void*,void*))
{
    void* auxElement= NULL;
    ...
    if(this != NULL && pFunc != NULL)
    {
    	...
    	if(listaClone != NULL)
    	{
			for(int i=0; i < ll_len(this);i++)
			{
				...
				{
					auxElement2 = ll_get(this2, j);

					if(!pFunc(auxElement,auxElement2))
					{
						ll_add(listaClone, auxElement);
					}
				}
